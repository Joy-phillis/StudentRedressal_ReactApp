...
export default function StaffReports({ navigation, route }: any) {
  ...
  const fetchStaffData = async () => {
    try {
      ...
      // Fetch assigned complaints with date filtering
  ...
  const metrics: ReportMetrics = useMemo(() => {
    ...
    const overdue = assignedComplaints.filter(c => c.status === 'Overdue').length;
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const count = assignedComplaints.filter(c => {
        ...
        <html>
          ...
          <body>
            ...
            <table>
              ...
              ${assignedComplaints.slice(0, 10).map(c => `
                <tr>
                  <td>${c.title}</td>
            ...
            <div class="footer">
              <p>Student Redressal System - Confidential Report</p>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={[styles.card, { marginTop: 16 }]}>
          ...
          {assignedComplaints.length === 0 ? (
            ...
            assignedComplaints.slice(0, 10).map((complaint, index) => (
